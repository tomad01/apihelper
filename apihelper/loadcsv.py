...
"""
import csv,sys,os,string,itertools,pdb,argparse,cx_Oracle
from six import string_types, text_type
from optparse import OptionParser
from StringIO import StringIO
from apihelper.apihelper import Wait_bar
__version__ = '0.4'
...
##        types = get_types(open(datafile))
        ...
        for ind,col in enumerate(cols):            
            if len(col[0])>30:
                col[0] = 'col_renamed'+str(ind)
            sql = sql + ('\n\t%s VARCHAR(%s),' % (col[0],typo[ind]))
        with open('temp_data','w') as f:
            f.write(sql[:len(sql)-1] + ')')
...
def output_sql_from_file(outfile, table, inputfile, placeholder="%s",
    ...
    iter_input = csv.reader(inputfile, dialect='excel', delimiter=delimiter)
    columns = iter_input.next()
    """ Output SQL to outfile reading input from iter_input """
    insert_sql = generate_sql_insert(table, columns, placeholder)
    cnt=0;rows=[]
    conn = cx_Oracle.connect('mesread_br/mesread_br@sudb920-vm:1521/messudwh')
    c = conn.cursor()
    with open('temp_data','r') as f:
        try:
            ...
            print str(er)
    iters = sum(1 for _ in itertools.ifilter(None, iter_input))
    waitbar = Wait_bar(iters/9000)
    inputfile.seek(0)
    try:
    ...
    iter_input.next()
    def_len = insert_sql.count('%s')
    for row in iter_input:
        cnt+=1
##        rows.append(tuple(["'"+i+"'" for i in row[0].replace('"','').replace("'",'').split(',')]))

            
        foo = tuple(["'"+i.replace('"','').replace("'",'')+"'" for i in row])
##        if len(foo)>def_len:
##            foo = foo[:-1]
        try:
            c.execute(insert_sql%foo)
        except Exception as er:
            print str(er)
            print cnt,foo
            pdb.set_trace()
