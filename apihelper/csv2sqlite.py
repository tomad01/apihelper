...
def convert(filepath_or_fileobj, dbpath, table, headerspath_or_fileobj=None, compression=None, typespath_or_fileobj=None):
    ...
    # now load data
    if not 'primary key' in types[0].lower():
        headers.insert(0,'id')
        types.insert(0,'INTEGER PRIMARY KEY AUTOINCREMENT')
    ...
    _insert_tmpl = 'INSERT INTO %s(%s) VALUES (%s)' % (table,','.join(['"'+ii+'"' for ii in headers]),
        ','.join(['?']*len(headers)))
